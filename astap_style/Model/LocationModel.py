...
class LocationModel(QAbstractTableModel):
    current_list = None
    current_indexes_list = None
    header = ["ширина", "длина"]
    culture_columns = ["width", "height"]
    ...
    def update_locations(self):
        ...
        for (id, name, amount) in cursor:
            self.current_list.append([name, amount])
    ...
    def check_column_value(self, column_id, value):
        if not value:
            self.show_message("Значение не должно бть пустое")
            return False
        if column_id == 1:
            if not re.fullmatch("\d*", value):
                self.show_message("Значение должно быть числом")
                return False
        return True
    ...
    def update_data(self, id, column_index, value):
        ...
        cursor.execute("Update Location set %s='%s' where id = %d" % (self.culture_columns[column_index], value, id))
        self.mysql_db.commit()

    Remove_Query = """
           delete from Location where id = %d
           """
